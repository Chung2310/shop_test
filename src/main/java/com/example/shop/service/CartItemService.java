...
public class CartItemService {
    ...
    public ResponseEntity<ApiResponse<CartItemDTO>> addItemToCart(CartItemRequest cartItemRequest) {
        ...
        if(cartItemRequest.getQuantity() > product.getQuantity()){
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body(new ApiResponse<>(HttpStatus.BAD_REQUEST.value(), "Số lượng sản phẩm còn lại không đủ!",null));
        };

        Optional<CartItem> cartItemOptional = cartItemRepository.findByUserIdAndProductId(
    ...
    public ResponseEntity<ApiResponse<CartItemDTO>> updateItemToCart(CartItemRequest cartItemRequest) {
        ...
        CartItem item = cartItemRepository.findByUserIdAndProductId(
                        cartItemRequest.getUserId(), cartItemRequest.getBookId())
                .orElseThrow(() -> {
    ...
    public ResponseEntity<ApiResponse<String>> deleteItem(Long userId, Long bookId) {
        ...
        CartItem item = cartItemRepository.findByUserIdAndProductId(userId, bookId)
                .orElseThrow(() -> {
